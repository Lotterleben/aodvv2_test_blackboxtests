...
const json_template_received_rrep = "{\"log_type\": \"received_rrep\", \"log_data\":{\"last_hop\": \"%s\", \"orig_addr\": \"%s\", \"orig_addr_seqnum\": %d, \"targ_addr\": \"%s\", \"targ_addr_seqnum\": %d}}"
...
func test_route_creation_0_to_3() {
    ...
    expected_json= fmt.Sprintf(json_template_received_rrep, end.Ip, beginning.Ip, 1, end.Ip, 1)
    riot_line[2].Channels.Expect_JSON(expected_json)
    /* TODO FIXME seqnum is 2 because apparently
    ...
    expected_json= fmt.Sprintf(json_template_received_rrep, riot_line[2].Ip, beginning.Ip, 1, end.Ip, 1)
    riot_line[1].Channels.Expect_JSON(expected_json)
    ...
    expected_json= fmt.Sprintf(json_template_received_rrep, riot_line[1].Ip, beginning.Ip, 1, end.Ip, 1)
    beginning.Channels.Expect_JSON(expected_json)
